...
**Version 0.3.0** · © 2026 Patrick Walter · [www.gummipunkt.eu](https://www.gummipunkt.eu) · [noteit@gummipunkt.eu](mailto:noteit@gummipunkt.eu) · Lizenz: [GPL 3.0](LICENSE)

Verfügbar auf Deutsch, Englisch, Französisch, Italienisch und Spanisch. · [English README](README.md)

Quellcode: <https://github.com/gummipunkt/NoteIt>

## Herunterladen

Lade die neueste `NoteIt-<Version>.dmg` von der Seite [Releases](https://github.com/gummipunkt/NoteIt/releases), öffne sie und ziehe **NoteIt** in den Ordner **Programme**.

NoteIt ist noch nicht von Apple notarisiert. Verweigert macOS beim ersten Start das Öffnen, gehe zu **Systemeinstellungen → Datenschutz & Sicherheit**, scrolle nach unten und klicke auf **Dennoch öffnen**. Alternativ entfernst du die Quarantäne-Markierung mit `xattr -dr com.apple.quarantine /Applications/NoteIt.app`.
...
- **Fünf Sprachen**: Deutsch, English, Français, Italiano, Español. NoteIt folgt der Systemsprache, unter Einstellungen → Allgemein kannst du eine andere wählen.
...
# … und daraus build/NoteIt-<Version>.dmg packen
scripts/make-dmg.sh
```
...
Jeder Push baut die App außerdem per GitHub Actions. Das DMG und Bildschirmfotos (eins pro Sprache) liegen dann als Artefakte am jeweiligen Workflow-Lauf.

Die Versionsnummer steht an genau einer Stelle, in `Sources/NoteIt/AppInfo.swift`. Das Build-Skript übernimmt sie ins App-Bundle, die Build-Nummer ist die Anzahl der Commits. Änderungen stehen im [CHANGELOG](CHANGELOG.md) (auf Englisch). Wird ein Versions-Tag gepusht (`git tag -a v0.3.0 -m "NoteIt 0.3.0" && git push origin v0.3.0`), erstellt die CI zusätzlich ein GitHub-Release mit dem DMG und dem passenden CHANGELOG-Abschnitt.
...
Sources/NoteItCore/        plattformunabhängige Logik (läuft und testet auch unter Linux)
  ...
  MarkdownSyntax.swift     Zerlegung für die Hervorhebung im Editor
  MarkdownRenderer.swift   Markdown → HTML (swift-markdown / cmark-gfm)
  L10n.swift               alle Texte der Oberfläche in fünf Sprachen
